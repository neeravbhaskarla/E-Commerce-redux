    ...
    return(
        <div className="flex flex-row font-roboto font-light">
            <div className="flex flex-col md:px-10 py-8 md:py-20 ml-3 md:ml-10 space-y-6 md:space-y-14" style={{width: '60vw', height:'70vh'}}>
                <div className="capitalize text-left md:text-4xl font-thin font-poppins">
                    {product.title}
                </div>
                <div className="text-xs md:text-sm font-light text-gray-500 text-left ml-2">
                    {product.description}
                </div>
                <div className="text-sm md:text-md font-bold text-gray-800 text-left md:ml-2">
                ...
                <div className="font-bold text-left md:text-right md:mr-24 text-sm md:text-lg">
                    Price: <span className="font-light ml-1">${product.price}</span>
                </div>
                <div className="flex flex-row justify-between items-center px-4">
                                <div className="flex items-start mt-3 mb-3 md:mt-0 md:mb-0">
...
const Button =(props) =>{
    return <button className="capitalize text-xs w-24 md:w-32 py-1 md:py-2 px-1 md:px-3 md:text-sm bg-black hover:bg-gray-400 text-white -ml-3 font-roboto font-normal transition-colors duration-500" onClick={props.clicked}>{props.children}</button>
}
